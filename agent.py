    ...
    for iteration in range(0, iterations):
        for state in states:
            ...
            for action in actions:
                next_states_probs = mdp.getTransitionStatesAndProbs(state, action)
                action_value = 0
                Q_value = {'north': 0, 'east': 0, 'south': 0, 'west': 0}
                for (next_state, prob) in next_states_probs:
                    reward = mdp.getReward(state, action, next_state)
                    Q_value_of_next_state = Q_values[next_state]
                    value_of_next_state = values[next_state]
                    action_value += prob * (reward + discount * value_of_next_state)
                    Q_value += prob * (reward + discount * Q_value_of_next_state)
